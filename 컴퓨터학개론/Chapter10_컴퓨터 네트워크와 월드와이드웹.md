...
+ 도메인 주소 : 실질적으로 통신되기 위해서는 IP주소로 통신해야 하는데, 우리나라에서는 인터넷 진흥원에서 관리를 한다고 한다. (원래는 InterNIC 에서 받는다)

+ URL(Uniform Resource Locator) : 인터넷상 표준 주소 표기 방법
  - 프로토콜://컴퓨터이름.소속단체.단체성격.소속국가/호스트 내부위치 
  - http가 있고 https가 있음. (s : secure)

### TCP Segment 헤더 

<img src = "images/CompStart_Ch10_9.png" />

+ Port 번호(16bit) : 상위(응용)계층에서 돌아가는 프로세스가 하나만 돌아갈 리가 없다. 
  - 여러 프로그램이(프로세스가) 동시에 돌아가게 된다. 
  - 상위 계층의 프로세스를 식별하기 위해 Port 번호가 들어가게 된다. 
  - ex) http의 경우, 포트 번호 80, ftp 의 경우, 포트번호 20,21 이런식이라고 한다
  - (기억상 4자리인데 ㄹㅇㅋㅋ)

<img src = "images/CompStart_Ch10_10.png" />

+ 웹 서비스를 할려면, 반드시 서버가 있어야 함. 웹 서버 쪽에 모든 정보를 담아두고, 클라이언트 쪽에서 정보를 요청하면, 저장해 놓았다가 정보를 제공한다.

+ DNS 서버가 전 세계의 모든 도메인에 해당하는 IP 주소를 가지고 있는것은 아니다!
  - DNS 서버가 다른 DNS서버와 연결되어 있는 식으로, 계층 구조로 알아내어 IP 주소를 가르쳐주게 된다.

### 웹 브라우저(Web Browser) 

+ 웹 정보를 쉽게 참조할 수 있는 프로그램
+ 모자이크 : 최초의 웹 브라우저
+ 네비게이터 : 1994년 넷스케이프 커뮤니케이션에서 발표
  - 네비게이터 1.0을 Mozilla라는 이름으로 알려짐

+ IE(Internet Explorer) 와 엣지(Edge) 
  - 1995년 MS사가 IE 발표, ActiveX 컨트롤러와 Java Applet을 실행할 수 있는 환경을 제공함
  - Java Applet : 웹 페이지 않에 간단한 기능을 하는 프로그램
    - ActiveX 가 웹 페이지 안에서 실행 파일을 IE 안에서 실행할 수 있게끔 해줌.
  - 2015년 MS에서 Edge라는 브라우저를 발표함. HTML 5를 사용하고, ActiveX 문제를 해결하기 위해서 만듬.

+ FireFox 
  - 넷스케이프 커뮤니케이션(네비게이터 만든 회사) 이 퇴보되어, Mozilla 라는 비영리 재단에서 이 회사를 인수해서 개발한 웹 브라우저

+ Chrome : 전 세계적으로 가장 많이 사용하는 웹 브라우저

+ Safari : 2004년 애플이 발표, 애플의 맥/아이폰/아이패드에서 사용

### 웹 서버

+ 웹의 HTTP를 사용하여 클라이언트의 요청에 응답하는 프로그램, 서버 컴퓨터에 항상 실행되어 있어야 함.

+ IP가 고정적으로 있어야 함. 유동적으로 변하면 사용할 수 없다. 
+ Apache, Flask 같은 웹 서버 프로그램을 설치하면 웹 서버 컴퓨터가 되는 것이다.

+ Apache 
  - Apache S/W foundation 에서 오픈소스로 개발함.(무료)
 
+ 엔진엑스(Nginx) : 
  - 러시아의 이고르 시셰프 가 개발한 오픈소스 웹서버
  - 오픈소스 이기 때문에 무료로 사용할 수 있는 웹 프로그램이다
  - 1위를 차지하고 있다고 한다. 
  - 페이스북, 넷플릭스, 깃허브, 네이버 등에서 사용된다.

### 웹 프로그래밍(모르는것만 적자)
+ DTD(Data Type Definition) : HTML의 DTD는, HTML에서 이용가능한 태그를 정의한 문서이다.
+ D(Dynamic)HTML : 글자에 마우스 포인터를 갖다대면, 글자 색깔이 변하는 그런 것도 있다고 한다.
  - 이거 HTML 안에서 CSS로 다 처리할 수 있는것같은데 ㄹㅇㅋㅋ

+ XML은, HTML 에 없는 태그를 설정해 이용가능하다. (마찬가지로, DTD라는 곳에 정의해 사용한다고 한다.)

+ XSL(XML Stylesheet Language) : 문서 내용과 디자인을 분리가능하다고 한다.(안드로이드에서는 다 XML에서 했던것같은데 ㄹㅇㅋㅋ)

+ 웹 문서를 전달함으로 인해, 그냥 단순히 보여주기 위한 웹 문서를 전달하는것이 아니라, 웁 문서 안에 들어가는 데이터들을 다른쪽으로 응용할 수 있도록, 데이터들을 XML 형식으로 표현해 두면 웹 서버를 구축하는데 있어서도 도움이 되지만, 포털 사이트에서 각 웹페이지에 있는 모든 내용을 파악하는데도 더 도움디 된다고 한다.. 일단 그렇다고 한다... 

+ ASP(Active Server Page) : VBScript를 HTML 문서에 코딩하여 동적 웹페이지를 구현하는 기술.

+ CGI(Common Gateway Interface) : 웹서버와 응용 프로그램 사이에서 정보를 주고 받는 방법/규약. 서버에서 많은 프로그램을 수행하여 서버에 부담이 간다.

+ VBScript (& JS) : 컴파일 없이 웹 브라우저에서 실행가능한 언어. VBScript는 MS 사아 Visual Basic을 기초로 만든 스크립트 언어라고 한다.

+ Servlet : 동적인 웹 페이지를 만들 때 사용되는 자바 기반의 웹 애플리케이션 프로그래밍 기술이다. 
  - Apache 같은 웹 서버 프로그램도 설치가 되지만, Servlet이 모든 데이터를 처리하기도 한다.

+ JSP(Java Server Page) : html 안에 로직을 삽입할 수 있는 추가적인 방식이 필요해 개발된 프로그램.
  - html 태그 안에, 중간중간 자기 코드(java)를 집어넣는 방식이다.
  - 웹 서버에서 JSP를 실행시키려면, 자바 엔진이 필요하다. 

+ Tomcat : 자바 엔진의 대표적인 예시. Apache 재단에서 개발한 무료 웹 서버이다.

+ Node.JS : 서버 사이드에서 JS를 사용하기 위해 개발되었다. 
  - 웹 서버를 구축하고, 웹 서버를 관리하는데 잇어 코드 유지, 수정, 보수가 편리하다
  - Node.JS를 이용해 많이 서버를 개발한다.
  - HTML 라이브러리는 모두 이미 포함하고 있기 때문에, 웹 서버에 아파치 같은 서버 프로그램을 별도로 설치할 필요가 없다는 장점이 있다.

### 인터넷 검색포털

+ 검색엔진의 원리 : 평상시 전 세계 사이트 웹 페이지를 방문해 정보를 수집합(Crawling)
+ 의미 있는 단어들을 빼내어서 DB에 저장한다(Indexing)
+ 그래서, 사람들이 검색했을 때, 키워드와 관련된 여러 가지 적합도, 중요도 등을 고려해서 결과를 보여주게 된다.

### 인터넷 기술 변화

+ 2010년대 이후, 웹 3.0을 사용하고 있다. 

+ 웹 2.0 : 
  - 모든 사람이, 제공되고 있는 데이터를 활용해 다양한 서비스를 생산해 낼 수 있는 웹 환경을 제공한다.
  - ex) 구글 맵을 이용한 부동산 사업 / 여행 안내 사업 등을 웹을 이용해서 할 수 있다.

+ 웹 3.0 
  - 개인 중심의 서비스로, 창의성이 최대한 발휘될 수 있는 웹 환경을 제공하고 있음.
  - 네이버 포털 사이트에서, GPS를 읽어서 대현동의 날씨를 가르쳐준다(검색안해도)
