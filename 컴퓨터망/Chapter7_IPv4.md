...
+ 3번째 Line의 제일 오른쪽에 있던 애. Size : 16bit
+ header 부분의 값을 다 더한 값에 보수를 취해서, checksum 까지 합쳤을 때 0 이 되도록 한 concept.
+ 다 더해서 0 이 나오면 문제가 없는 것이고, 0이 아니라는 이야기는 어딘가 문제가 생겼다는 의미이다.
+ 오류가 생겼는지 안생겼는지 16bit 단위로 간단히 체크할 수 있다. 
+ 보내는 애는 checksum 만들어서 보내고, 받는 애는 그것까지 다 더해서 0이 나오면 된다. 
+ 다 더했는데 0이 아니면 폐기해야 한다. 그만큼 header 정보가 중요한데, 바뀌면 의미가 없기 때문.
+ 헤더만 컨트롤하는 것임. Data하고는 무관하다. 
