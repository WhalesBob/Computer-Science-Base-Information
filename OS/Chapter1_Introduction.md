...
+ Time-Sharing Process(1970s~1980s)
  ...
  - 혹은, 10명의 사용자가 있다면, 순차적으로 문제를 그냥 온대로 다 끝내고 그 다음 부분으로 나아가고자 했다면, 맨 마지막에 CPU를 배정받는 사람은 엄청 기다려야 할 것이다. 
  - 할당받을 수 있는 CPU의 Computing 능력을, CPU를 점유할 수 있는 시간, 기회를 균등하게 나누어 주자! (1ms씩 나누어 주면 될 수 있다) 
    - ex) 1+1의 연산을 컴퓨터 상의 계산기에게 수행하라고 했을 때, 그냥 0.1초 안에 결과가 나온다면 사용자는 그 결과가 바로 나왔다고 생각한다. 굳이 1ms 안에 찍어 줄 필요가 없다. 어차피 사람들은 그 차이를 못 느낀다. 그러므로, 굳이 0.1초보다 더 안쪽으로 결과를 낼 필요가 없다는 결론이 나온다. 
    - 실제로 10명이 사용하는데, 0.01초 만에 그 결과를 낼 수 있다고 치면, 앞의 9명에게 각 0.01초만에 결과를 다 뿌리고 돌아와서, 이 사람에게 0.1초만에 결과를 줘도 상관없는 것이다. 
  - 실제로도 이렇게 한다! 
    - CPU의 속도, 하드웨어의 속도가 워낙 빨라서, 사람이 입력을 하거나 아웃풋을 해석하는 시간 동안 충분히 다른 것들을 다 수행할 수 있기 때문에 가능하다. 
    - 실제로는 1 nanosecond 동안 한개의 일이 수행되는 것인데, 사용자는 CPU를 온전히 다 사용하고 있다고 생각하게 되는 것이다(실제로는 10명이 사용하고 있더라도) 
    - 10명이 사용하든 100명이 사용하든, Time Sharing 으로 인해서 각 User들이 CPU를 본인 혼자서 사용하고 있다고 "착각"하게 만드는 것이 Time-Sharing의 핵심이다. 어차피 1ns안에 다 오는데, 사람의 인식범위 안에서는 0.1초만에 결과가 나와도, 본인에게 결과가 지체없이 왔다고 생각할 수 밖에 없기 때문.
  ...
  - 왜 Time-Sharing의 목적은 "Response Time을 줄이는 것" 인가?
    ...
    - 이렇게 각각의 사용자에게 나눠서, 그 사용자가 바로바로 결과를 제공받는다고 생각하게 만들어, 사용자 경험을 증대시키는 것이 주 목적이다. 
  
  - __Context Switching__ : Round Robin 방법에서 A에서 B로 Job 을 바끌 때 일어나는 switching.
    - 너무 자주 Context Switching이 일어나면, 오히려 이 Context Switching에 들어가는 시간으로 인해 전체 Response Time이 줄어들 위험이 있다. 
